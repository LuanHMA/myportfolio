...
export function Header() {
  ...
  return (
    <>
      <header
        className="fixed left-0 top-0 right-0 bg-slate-50 flex justify-between items-center py-2 px-6 z-50 shadow-lg"
      ...
      >
        ...
        <ul
          className={`fixed bg-slate-50 top-14 w-[40vw] h-screen flex flex-col gap-y-4 px-6 shadow-lg z-50 transition-all duration-300 ${
          ...
          {menuItems.map((item, index) => {
            return (
              <li key={index} className="w-full relative group">
                ...
                <span className="absolute bottom-0 left-0 w-0 h-1 bg-blue-400 transition-all duration-200 group-hover:w-[60px]"></span>
              </li>
            );
          })}
        </ul>
